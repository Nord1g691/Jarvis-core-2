...
(async () => {
  ...
  const inject = (hud) => {
    ...
    if (!root.getElementById("ledBrightnessCard")) {
      ...
      const controls = [...grid.querySelectorAll(".card")].find(x => x.querySelector("#refresh")) || grid.lastElementChild;
    ...
    if (select && !select.dataset.bound) {
      ...
      select.value = localStorage.getItem("jarvis_assist_pipeline") || "";
      select.onchange = () => {
        localStorage.setItem("jarvis_assist_pipeline", select.value);
        hud._log?.("✓ Pipeline → " + (select.value || "AUTOMATIQUE"));
      };
      hud._loadJarvisPipelines?.();
    }

    const bright = root.getElementById("ledBrightness");
    if (bright && !bright.dataset.bound) {
      bright.dataset.bound = "1";
      bright.value = localStorage.getItem("jarvis_led_brightness") || "100";
      const apply = () => { const v = Number(bright.value); localStorage.setItem("jarvis_led_brightness", String(v)); hud.style.filter = `brightness(${v}%)`; };
      bright.oninput = apply;
      apply();
    }
    return true;
  };

  C.prototype._loadJarvisPipelines = async function () {
    const root = this.shadowRoot;
    const select = root?.getElementById("pipelineSelect");
    const info = root?.getElementById("pipelineInfo");
    if (!select || !this._hass?.callWS) return;
    try {
      ...
      for (const p of pipelines) {
        if (!p?.id || [...select.options].some(o => o.value === p.id)) continue;
      ...
      if (info) info.textContent = pipelines.length ? `${pipelines.length} pipeline(s) Assist détecté(s)` : "Aucun pipeline Assist détecté";
      this._log?.("✓ Assist · " + (pipelines.length || 0) + " pipeline(s)");
    } catch (e) {
      if (info) info.textContent = "Détection Assist indisponible";
  ...
  C.prototype.process = async function (text) {
    ...
    try {
      ...
      this._log("✓ Assistant Assist → " + (d.agent_id || "HA") + " · " + (d.pipeline_name || "Automatique"));
      const speech = d?.response?.speech?.plain?.speech || d?.response?.speech?.ssml?.speech || "";
      if (speech) {
        this._log("🤖 " + speech);
        if (!this.muted) {
          this.setState("JARVIS PARLE", "#b56cff");
          const u = new SpeechSynthesisUtterance(speech); u.lang = "fr-FR"; u.rate = .92;
  ...
  const timer = setInterval(() => { document.querySelectorAll("jarvis-core-hud").forEach(inject); }, 100);
